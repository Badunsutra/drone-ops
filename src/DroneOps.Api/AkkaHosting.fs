...
let configureAkka (services: IServiceCollection) =
    // world создаётся здесь — до AddAkka
    // регистрируем в DI чтобы WorldEndpoints мог получить его через ctx.GetService<WorldMap>()
    let world = buildInitialWorld ()
    services.AddSingleton<WorldMap>(world) |> ignore
        ...
        System.Action<AkkaConfigurationBuilder, System.IServiceProvider>(fun builder sp ->
            builder
                ...
                .WithActors(fun system (registry: IActorRegistry) ->
                    // world захватывается из внешней области видимости — не пересоздаём
                    ...
                    let chargingActor =
                        system.ActorOf(
                            Props.Create<ChargingCoordinatorActor>(fun () -> ChargingCoordinatorActor(world)),
                            Charging
                        )
                    ...
                    // 6. TelemetryActor
                    let telemetryActor =
                        system.ActorOf(Props.Create<TelemetryActor>(fun () -> TelemetryActor(150)), Telemetry)

                    registry.Register<TelemetryActor>(telemetryActor)

                    // 7. SignalRBridgeActor
                    let hubContext = sp.GetRequiredService<IHubContext<WorldHub>>()

                    let signalRBridge =
                        system.ActorOf(
                            Props.Create<SignalRBridgeActor>(fun () -> SignalRBridgeActor(hubContext)),
                            SignalRBridge
                        )

                    registry.Register<SignalRBridgeActor>(signalRBridge)
                    // Даём TelemetryActor время подписаться на EventStream
                    // перед тем как дроны опубликуют DroneSpawned в PreStart
